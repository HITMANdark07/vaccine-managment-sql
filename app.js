...
function check_priority(date){
  if((new Date(Date.now()).getFullYear()- new Date(date).getFullYear())>=60){
    return "Yes";
  }else{
    return "No";
  }
...
app.get("/choose_hosp/:pin/:pid", (req, res) => {
  ...
  //   pri = result;
  // });
  con.start.query("select P_DOB from Person where P_id = ?;", req.params.pid, function(err,result) {
    if(err) throw err;
    pri=check_priority(result);
  })
